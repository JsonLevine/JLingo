...
function validateGuess(input){
	for(var i =0; i< possibleWords.length; i++){
		if(input == possibleWords[i].toUpperCase()){
	...
	document.getElementById("solution_display").innerHTML ="'" + input + "' isn't a possible word here.  Please try another ";
	return false;
...
function submitGuess(row){
	...
	validGuess = validateGuess((currentGuess + "").replace(/,/g,''));
