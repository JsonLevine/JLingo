...
const dictionary =  allWords1.concat(allWords2).concat(allWords3).concat(possibleWords);

var currentRow = 0; //row the user is currently guessing (0-5)
var correctLetters = 0; //number of letters the user has guessed correctlky so far (0-5)
var validGuess = true; //if the current guess is a real word (default true)
...
function validateGuess(input){
	...
	for(var i =0; i< dictionary.length; i++){
		if(input == dictionary[i].toUpperCase()){
	...
	//tell user their guess isn't in the dictionary if it isnt found
	document.getElementById("solution_display").style.display = "block";
	document.getElementById("solution_display").innerHTML ="'" + input + "' doesn't seem to be a valid word.  Please try another ";
	return false;
...
function submitGuess(row){
	...
	validGuess = validateGuess(convertToString(currentGuess));
	...
	if(validGuess){
		...
		for(var n=0; n<5; n++){
			if(solutionCopy[n] == currentGuess[n]){
				...
				colorKeyboardTile(currentGuess[n], greyTile);

				//make the font lighter too for grey ones
				document.getElementById(currentGuess[n]).style.color = greyTileFont
			...
			if(correctLetters == 5){
				...
				//user wins
				endGame(true, currentRow);

				//make the current row unachievably high
				currentRow = 999;
			}
		}
...
function endGame(winner, endRow){

	//disable final row
	disableRow(endRow);

	//convert solution back to a string to display
	solution = convertToString(solution)
...
function updateStreak(win){
	...
	//game was won, iterate on streak
	if(win){
		currentStreak++;
	}

	//game was lost, reset streak to 0
	else{
		currentStreak = 0;
	}
...
function colorTile(row, column, color) {
	document.getElementById("form"+row+"_"+column).style.background = color
}

function colorKeyboardTile(letter, color) {
...
function disableRow(row) {
	...
	for(var column=0; column <5; column++){
		...
		document.getElementById("input"+row+"_"+column).disabled = true;
	}
...
function enableRow(row) {
	...
	for(var column=0; column <5; column++){
		...
		document.getElementById("input"+row+"_"+column).disabled = false;
	}
}
